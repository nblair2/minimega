...
`,
	...
	},
	{
		...
		HelpLong: `Assign a VM state image, generated by a previously saved VM to boot
with. Explicitly, this is the state of a previously running VM and NOT the disk state.
By default, images are read from the files directory as specified
...
`,
		...
		Call: wrapSimpleCLI(func(ns *Namespace, c *minicli.Command, r *minicli.Response) error {
			if len(c.StringArgs) == 0 {
				r.Response = ns.vmConfig.StatePath
				return nil
			}

			v := checkPath(c.StringArgs["value"])

			ns.vmConfig.StatePath = v
...
Requires QEMU 6.1+ compiled with spice and for spice-vdagent to be installed on VM.
